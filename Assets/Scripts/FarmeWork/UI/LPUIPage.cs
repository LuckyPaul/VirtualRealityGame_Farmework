namespace LuckyPual.UI
{
    ...
    public abstract class LPUIPage
    {
        ...
        private static Dictionary<string, LPUIPage> m_allPages;
        public static Dictionary<string, LPUIPage> allPages
        { get { return m_allPages; } }

        //control 1>2>3>4>5 each page close will back show the previus page.
        private static List<LPUIPage> m_currentPageNodes;
        public static List<LPUIPage> currentPageNodes
        ...
        private LPUIPage() { }
        public LPUIPage(UIType type, UIMode mod, UICollider col)
        {
            this.type = type;
            this.mode = mod;
            this.collider = col;
        ...
        {
            LPUIRoot.Instance.StartCoroutine(AsyncShow(callback));
        }
        ...
        {
            if (LPUIRoot.Instance == null || ui == null) return;
            ...
            {
                ui.transform.SetParent(LPUIRoot.Instance.fixedRoot);
            }
            else if (type == UIType.Normal)
            {
                ui.transform.SetParent(LPUIRoot.Instance.normalRoot);
            }
            else if (type == UIType.PopUp)
            {
                ui.transform.SetParent(LPUIRoot.Instance.popupRoot);
            }
        ...
        private static void ShowPage<T>(Action callback, object pageData, bool isAsync) where T : LPUIPage, new()
        {
            Type t = typeof(T);
        ...
        /// Sync Show Page
        /// </summary>
        public static void ShowPage<T>() where T : LPUIPage, new()
        {
        ...
        /// </summary>
        public static void ShowPage<T>(object pageData) where T : LPUIPage, new()
        {
        ...
        /// </summary>
        public static void ShowPage<T>(Action callback) where T : LPUIPage, new()
        {
            ShowPage<T>(callback, null, true);
        }

        public static void ShowPage<T>(Action callback, object pageData) where T : LPUIPage, new()
        {
        ...
        public static void ClosePage<T>() where T : LPUIPage
        {
            Type t = typeof(T);
            string pageName = t.ToString();
